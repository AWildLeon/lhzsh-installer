#!/usr/bin/env bash

set -euo pipefail
IFS=$'\n\t'

BASEURL="https://zsh.onfr.de"

if [ -z "$HOME" ]; then
    echo "Your \$HOME is empty, can't continue!"
    exit 1
fi

DSTPATH="$HOME/.frzsh"

log_info() {
    local message="$1"
    echo -e "$(date '+%Y-%m-%d %H:%M:%S') - \033[32mINFO: $message\033[0m" >&2
}

log_error() {
    local message="$1"
    echo -e "$(date '+%Y-%m-%d %H:%M:%S') - \033[31mERROR: $message\033[0m" >&2
}

detect_sudo_tool() {
    if [ "$(id -u)" -eq 0 ]; then
        echo ""
        log_info "Running as root, no sudo tool required."
    else
        if command -v sudo &>/dev/null; then
            echo "sudo"
        elif command -v doas &>/dev/null; then
            echo "doas"
        else
            log_error "No sudo tool found. Please install sudo or doas."
            exit 1
        fi
    fi
}

install_fastfetch_deb() {
    local arch package_name url sudo_tool
    arch=$(uname -m)

    case "$arch" in
    x86_64) package_name="fastfetch-linux-amd64.deb" ;;
    aarch64) package_name="fastfetch-linux-aarch64.deb" ;;
    armv7l) package_name="fastfetch-linux-armv7l.deb" ;;
    *)
        log_error "Unsupported architecture: $arch"
        return 1
        ;;
    esac

    log_info "Fetching latest Fastfetch release for $arch..."
    url=$(curl -s https://api.github.com/repos/fastfetch-cli/fastfetch/releases/latest | grep browser_download_url | cut -d\" -f4 | grep "$package_name")

    if [ -z "$url" ]; then
        log_error "Failed to retrieve the download URL for $package_name"
        return 1
    fi

    log_info "Downloading $package_name..."
    if ! curl -LSso "/tmp/$package_name" "$url"; then
        log_error "Download failed for $package_name"
        return 1
    fi

    sudo_tool=$(detect_sudo_tool)
    log_info "Installing $package_name..."
    if ! ${sudo_tool} apt-get install "/tmp/$package_name" -y; then
        log_error "Installation failed for $package_name"
        return 1
    fi

    log_info "Fastfetch installed successfully!"
}

detect_package_manager() {
    if command -v apt-get >/dev/null 2>&1; then
        echo "apt-get"
    elif command -v pacman >/dev/null 2>&1; then
        echo "pacman"
    elif command -v yum >/dev/null 2>&1; then
        echo "yum"
    elif command -v apk >/dev/null 2>&1; then
        echo "apk"
    else
        log_error "No supported package manager found."
        exit 1
    fi
}

install_packages() {
    log_info "Installing required packages..."

    local sudo_tool
    sudo_tool=$(detect_sudo_tool)

    local packages=("zsh" "git" "curl" "zoxide" "unzip" "jq" "eza")

    # Source OS release info for distro-specific fastfetch installation
    # shellcheck disable=SC1091
    . /etc/os-release 2>/dev/null || true

    case "${ID:-}" in
    debian)
        if [ "${VERSION_ID:-}" = "12" ]; then
            install_fastfetch_deb
        else
            ${sudo_tool} apt-get install -y fastfetch
        fi
        ;;
    ubuntu)
        if [ "${VERSION_ID:-}" = "24.04" ]; then
            install_fastfetch_deb
        else
            ${sudo_tool} apt-get install -y fastfetch
        fi
        ;;
    *)
        packages+=("fastfetch")
        ;;
    esac

    log_info "Installing common packages: ${packages[*]}"

    case $(detect_package_manager) in
    apt-get)
        ${sudo_tool} apt-get update && ${sudo_tool} apt-get install -y "${packages[@]}"
        ;;
    pacman)
        ${sudo_tool} pacman -Sy --noconfirm --needed "${packages[@]}"
        ;;
    yum)
        ${sudo_tool} yum install -y "${packages[@]}"
        ;;
    apk)
        ${sudo_tool} apk add "${packages[@]}"
        ;;
    esac
}

cleanup_old_zsh() {
    echo "Hello There!"
    echo "It seems like you have a pre-rewrite version of my ZSH config."
    echo "To install the rewrite, I need to clean some stuff up. THIS WILL DELETE ANY ZSH RELATED STUFF!"
    echo -n "Would you like to continue? (y/N): "
    read -r response
    if [[ "$response" =~ ^[yY]$ ]]; then
        echo "Proceeding with cleanup..."
        rm -rf ~/.zshrc ~/.zshrc.d ~/.oh-my-zsh ~/.p10k.zsh ~/.frzshver ~/.config/neofetch

        local sudo_tool
        sudo_tool=$(detect_sudo_tool)
        case $(detect_package_manager) in
        apt-get)
            ${sudo_tool} apt-get remove --purge -y cowsay fortunes fortunes-de fortunes-min lolcat neofetch
            ${sudo_tool} apt-get autoremove -y
            ;;
        pacman)
            ${sudo_tool} pacman -Rns --noconfirm cowsay fortune-mod lolcat neofetch
            ;;
        yum)
            ${sudo_tool} yum remove -y cowsay fortune-mod lolcat neofetch
            ;;
        apk)
            ${sudo_tool} apk del neofetch
            ;;
        esac
    else
        echo ""
        echo "Aborted cleanup. Installation will *NOT* proceed."
        echo "If you are coming from pre v1.0.0, please manually update by typing:"
        echo ""
        echo "bash <(curl https://zsh.onfr.de/zsh.sh)"
        exit 0
    fi
}

download_file() {
    local url="$1"
    local output="$2"
    if command -v curl >/dev/null; then
        curl -Ls -o "$output" "$url"
    elif command -v wget >/dev/null; then
        wget -O "$output" "$url"
    elif command -v fetch >/dev/null; then
        fetch -o "$output" "$url"
    else
        log_error "No download tool found (curl, wget, fetch). Please install one of them."
        exit 1
    fi
}

main() {
    log_info "Starting FR ZSH installation..."

    if [ -f ~/.frzshver ] || [ -d ~/.zshrc.d ]; then
        cleanup_old_zsh
    fi

    mkdir -p "$HOME/.local/bin"
    export PATH="$PATH:$HOME/.local/bin"

    install_packages && clear

    # Install Oh My Posh
    curl -s https://ohmyposh.dev/install.sh | bash -s -- -d ~/.local/bin
    mkdir -p ~/.cache

    # Install Oh My Zsh if not already installed
    if [ ! -d "$DSTPATH/oh-my-zsh" ]; then
        download_file https://raw.githubusercontent.com/ohmyzsh/ohmyzsh/master/tools/install.sh /tmp/OhMyZsh.sh
        chmod +x /tmp/OhMyZsh.sh
        ZSH="$DSTPATH/oh-my-zsh" /tmp/OhMyZsh.sh --unattended
        rm /tmp/OhMyZsh.sh
    fi

    mkdir -p "$DSTPATH"

    rm -f ~/.zshrc

    # Download configuration files
    log_info "Downloading configuration files..."
    download_file "$BASEURL/theme.omp.json" "$DSTPATH/theme.omp.json"
    download_file "$BASEURL/zshrc" "$HOME/.zshrc"
    download_file "$BASEURL/latest_version" "$DSTPATH/version"

    # Change default shell to Zsh only if necessary
    local current_user current_shell
    current_user=$(id -un)
    current_shell=$(getent passwd "$current_user" | cut -d: -f7)

    if [ "$current_shell" != "$(which zsh)" ]; then
        log_info "Changing default shell to Zsh..."
        chsh -s "$(which zsh)"
    else
        log_info "Default shell is already set to Zsh."
    fi

    clear

    echo "Thank you for choosing Flo's fork of Leon's ZSH Config."
    echo "Have an awesome day!"
}

main
